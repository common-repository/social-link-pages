...
class Admin extends Singleton {
	...
	public function import_link_pages() {
		...
		if ( empty( $handle ) === false ) {
			while ( ( $data = fgetcsv( $handle, null, "," ) ) !== false ) {
				...
				foreach ( $data as $k => $value ) {
					if (isset($fields[$k])) {
						$row[ $fields[ $k ] ] = sanitize_text_field( $value );
					}
				}
				$results[] = $row;
			}
		...
		foreach ( $results as $record ) {
			...
			$i = 0;
			if (!isset($record['post_name'])) {
				$record['post_name'] = sanitize_title( $record['post_title'] );
			}
			while ( isset($record['post_name']) && ! Db::instance()->is_slug_unique( $record['post_name'] ) ) {
				$i++;
				$record['post_name'] = sanitize_title( sprintf( '%s-%d', $record['post_name'], $i ) );
			}

			$record = array_map( 'sanitize_text_field', $record );
		...
		// Suppress output to avoid 'Cannot modify header information' errors
		ob_start();
		wp_redirect(
			add_query_arg(
				[
